...
at `5ea970b4`, the revision astrald `e85f5da4` requires) makes the width
...
Source citations are pinned to astral-go `5ea970b4` and astrald `e85f5da4`. The
...
(`mod/tcp/src/parse.go` `Module.Parse`, line 11 at astrald `e85f5da4`). The
endpoint's own `Network()` is `tcp` and never `inet`."""

KCP_NETWORK: Final = "kcp"
TOR_NETWORK: Final = "tor"
GATEWAY_NETWORK: Final = "gw"
"""`mod/gateway/src/module.go` `NetworkName`, line 28 at astrald `e85f5da4`.
The module is `gateway` and its network is `gw`."""

TOR_DEFAULT_PORT: Final = 1791
"""`mod/tor/src/module.go` `defaultListenPort`, line 17 at astrald `e85f5da4`.
...
class TorEndpoint(Endpoint):
    ...
    def parse(cls, text: str) -> "TorEndpoint":
        ...
        (`mod/tor/src/parse.go` `Parse`, astrald `e85f5da4`) and is accepted
...
class GatewayEndpoint(Endpoint):
    ...
    **That makes this the one type whose framings disagree about a value rather
    ...
    astrald's parser does three things this one cannot: it resolves each half
    through the directory, it refuses an endpoint whose gateway is its target,
    and it refuses one where either half resolves to the zero identity, which is
    what `""` and `anyone` resolve to (`mod/gateway/src/parser.go`
    `Module.Parse`, astrald `e85f5da4`).
    All three need a node. A wire type has none, so `parse` is astral-go's
    ...
    def address(self) -> str:
        ...
        (`astral/identity.go`, astral-go `5ea970b4`).
        """
...
anything else (`api/nodes/node_info.go`, astral-go `5ea970b4`). No other type
