...
`api/user/see_swarm_action.go` at astral-go `5ea970b4`, the revision astrald
`e85f5da4` pins). On `26bb51d5`, `objects.new` answers `nil` for the three old
...
`5ea970b4`). An `identity=` carrying no value therefore reaches
...
**`user.list_siblings`'s `zone` is inert on astrald `e85f5da4`.** It builds a
...
**`user.sync_with` declares no `start` on astrald `e85f5da4`.** Its argument
...
`Editor.SetMany` at astral-go `5ea970b4`), so a `start` sent there is accepted and
...
no active contract (`mod/user/src/op_accept_contract.go` at `e85f5da4`, the
...
(`mod/user/src/op_new_node_contract.go` at `e85f5da4`). Verified on an unclaimed
...
`e85f5da4` and astral-go `5ea970b4`, the revisions `tests/reference.py` pins;
...
class AdminSwarmAction(Action):
    ...
    (`api/user/admin_swarm_action.go` at astral-go `5ea970b4`).
...
class User(ModuleClient):
    ...
    async def list_siblings(
        ...
        argument is inert on astrald `e85f5da4` -- the context built from it is
    ...
    async def sync_with(
        ...
        **`start` reaches no code on astrald `e85f5da4`.** `syncAssets` takes
...
def new_node_contract_local(
    ...
    *,
    ...
    order (`api/user/contract.go` at astral-go `5ea970b4`, `NewNodeContract`).
