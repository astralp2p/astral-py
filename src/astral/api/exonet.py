...
`api/exonet/module.go` at astral-go `5b1d282` declares one interface and nothing
...
it. `mod/exonet/module.go` at astrald `d5bb0bbd` declares a per-network registry
...
`inet` (`mod/tcp/src/parse.go` `Module.Parse`, line 11 at astrald `d5bb0bbd`).
...
def _require(types: EndpointTypes, network: str) -> type[Endpoint]:
    ...
    (`mod/exonet/errors.go`, astrald `d5bb0bbd`). This SDK adds no exception
