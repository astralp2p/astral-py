...
`api/exonet/module.go` at astral-go `6ea26c7` declares one interface and nothing
...
it. `mod/exonet/module.go` at astrald `26bb51d5` declares a per-network registry
...
`inet` (`mod/tcp/src/parse.go` `Module.Parse`, line 11 at astrald `26bb51d5`).
...
def _require(types: EndpointTypes, network: str) -> type[Endpoint]:
    ...
    (`mod/exonet/errors.go`, astrald `26bb51d5`). This SDK adds no exception
