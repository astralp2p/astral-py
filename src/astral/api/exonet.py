...
`api/exonet/module.go` at astral-go `5ea970b4` declares one interface and nothing
...
it. `mod/exonet/module.go` at astrald `e85f5da4` declares a per-network registry
...
`inet` (`mod/tcp/src/parse.go` `Module.Parse`, line 11 at astrald `e85f5da4`).
...
def _require(types: EndpointTypes, network: str) -> type[Endpoint]:
    ...
    (`mod/exonet/errors.go`, astrald `e85f5da4`). This SDK adds no exception
