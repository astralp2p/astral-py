...
`993ffac0`), and an omitted `id` makes the op read `object_id.sha256` objects
...
`bf8542a`). `index()` is the single form and `index_many()` the batch. Verified
...
`bf8542a`). A body-input op whose reader omits an `eos` branch answers the same
...
`api/auth/action.go` at astral-go `bf8542a` declares
...
ahead of its container dispatch (`astral/blueprint_reflect.go` at `bf8542a`), so
